...
const LiveActivityPage = ({ user, token }) => {
  ...
  const fetchLiveActivity = async (showLoading = true) => {
    try {
      ...
      const response = await fetch(`${API_ENDPOINTS.BASE}/api/live-activity?${params}`, {
        headers: {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await fetch(`${API_ENDPOINTS.BASE}/api/live-activity/analytics`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      'ok': { color: 'bg-gray-100 text-gray-800', icon: CheckCircle }
    }
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-sm text-blue-600">
                          {truncateText(event.tracking_id, 15)}
                        </span>
                        {getStatusBadge(event.status)}
                      </div>
                      <div className="text-xs text-gray-600">
                        <div>📧 {event.auto_grabbed_email}</div>
                        <div>🌍 {event.ip_address} • {event.country}, {event.city}</div>
                        <div>🏢 {truncateText(event.isp, 20)}</div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left p-2 font-medium text-gray-600">ISP</th>
                  ...
                  <tbody>
                    {events.map((event) => (
                      <tr key={event.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-2">
                          <span className="font-mono text-sm text-blue-600">
                            {truncateText(event.tracking_id, 12)}
                          </span>
                        </td>
                        ...
                        <td className="p-2 text-sm">{event.auto_grabbed_email}</td>
                        <td className="p-2 text-sm">{event.country}</td>
                        <td className="p-2 text-sm">{event.city}</td>
                        <td className="p-2 text-sm">{truncateText(event.isp, 15)}</td>
